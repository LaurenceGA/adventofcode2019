...
func main() {
	...
	max := getMaxFeedbackAmplifierSignal(day7.GetInputProgram())
...
func getMaxFeedbackAmplifierSignal(prog []int) int {
	var max int
	for a := signalLower; a < signalUpper; a++ {
		for b := signalLower; b < signalUpper; b++ {
			if b == a {
				continue
			}
			for c := signalLower; c < signalUpper; c++ {
				if c == a || c == b {
					continue
				}
				for d := signalLower; d < signalUpper; d++ {
					if d == a || d == b || d == c {
						continue
					}
					for e := signalLower; e < signalUpper; e++ {
						if e == a || e == b || e == c || e == d {
							continue
						}
						out := runFeedbackAmplifier(prog, amplifierInput{
							a: a,
							b: b,
							c: c,
							d: d,
...
func runAmplifier(prog []int, phaseSettings amplifierInput) int {
	...
	wg.Add(5)
	go runProgram(&wg, progA, inA, aToB)
	go runProgram(&wg, progB, aToB, bToC)
	go runProgram(&wg, progC, bToC, cToD)
	go runProgram(&wg, progD, cToD, dToE)
	go runProgram(&wg, progE, dToE, outE)
...
func runFeedbackAmplifier(prog []int, phaseSettings amplifierInput) int {
	progA := copyProgram(prog)
	progB := copyProgram(prog)
	progC := copyProgram(prog)
	progD := copyProgram(prog)
	...
	var wg sync.WaitGroup
	wg.Add(5)
	go runProgram(&wg, progA, eToA, aToB)
	go runProgram(&wg, progB, aToB, bToC)
	go runProgram(&wg, progC, bToC, cToD)
	go runProgram(&wg, progD, cToD, dToE)
	go runProgram(&wg, progE, dToE, eToA)

	// Phase settings
	eToA <- phaseSettings.a
	aToB <- phaseSettings.b
	bToC <- phaseSettings.c
	cToD <- phaseSettings.d
	dToE <- phaseSettings.e
...
func runProgram(wg *sync.WaitGroup, prog []int, input <-chan int, output chan<- int) {
