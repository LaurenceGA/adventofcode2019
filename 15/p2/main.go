...
func distToTarget(in chan<- int, out <-chan int, startCoord coord, stopAtTarget bool) {
	curPos := startCoord
...
func dirIsValid(in chan<- int, out <-chan int, direction movementDirection) bool {
	...
	default:
		...
		newStatus := <-out
		if newStatus != movedToEmptySpace && newStatus != movedToTarget {
