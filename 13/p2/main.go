...
func inputVal(inChan chan int, val, times int) {
	for i := 0; i < times; i++ {
		time.Sleep(waitTime)
		inChan <- val
	}
...
func drawScreen(s tcell.Screen, out <-chan int) {
	for {
		x := <-out
		y := <-out
		z := <-out
		if x == -1 && y == 0 {
			s.SetContent(0, 0, '!', getScoreString(z), tcell.StyleDefault)
			continue
		}
		...
		s.SetContent(x, y+1, char, nil, tcell.StyleDefault)
		s.Show()
	}
}

func getScoreString(score int) []rune {
	return append([]rune("SCORE: "),
		rune('0'+digitAt(score, 5)),
		rune('0'+digitAt(score, 4)),
		rune('0'+digitAt(score, 3)),
		rune('0'+digitAt(score, 2)),
		rune('0'+digitAt(score, 1)),
		rune('0'+digitAt(score, 0)),
	)
}
