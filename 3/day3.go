...
// MovementDirection is one of four directions
type MovementDirection string

// The directions of movement
const (
	Up    MovementDirection = "U"
	Down  MovementDirection = "D"
	Left  MovementDirection = "L"
	Right MovementDirection = "R"
)

// Movement represnets a single path segment
...
func (m *Movement) String() string {
	return string(m.direction) + strconv.Itoa(m.distance)
}
...
// GetWires returns the input wires
func GetWires() (*Wire, *Wire) {
	inputPaths := strings.Split(input, "\n")

	return &Wire{path: constructPath(inputPaths[0])}, &Wire{path: constructPath(inputPaths[1])}
}

func constructPath(pathInput string) []*Movement {
	movementsInstructions := strings.Split(pathInput, ",")
	movements := make([]*Movement, 0, len(movementsInstructions))

	for _, inst := range movementsInstructions {
		...
		movements = append(movements, &Movement{
			direction: MovementDirection(inst[:1]),
			distance:  dist,
		})
	}
...
// Coordinate represents a point relative to the central port
type Coordinate struct {
	X, Y, Steps int
}

// ManhattanDistance is the sum of the absolute constituent coordinates
func (c *Coordinate) ManhattanDistance() int {
	return int(math.Abs(float64(c.X)) + math.Abs(float64(c.Y)))
}

func (c *Coordinate) String() string {
	return fmt.Sprintf("(%d, %d: %d)", c.X, c.Y, c.Steps)
}

// Rasterize converts a wire to a list of points
func (w *Wire) Rasterize() []*Coordinate {
	curX, curY, steps := 0, 0, 0
	var points []*Coordinate
	for _, m := range w.path {
		switch m.direction {
		case Up:
			for i := curY; i < curY+m.distance; i++ {
		...
		case Down:
			...
			steps += m.distance
		case Left:
			for i := curX; i > curX-m.distance; i-- {
				points = append(points, &Coordinate{
					X:     i,
					Y:     curY,
					Steps: steps + (curX - i),
