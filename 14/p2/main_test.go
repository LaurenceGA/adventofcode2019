...
5 BHXH, 4 VRPVC => 5 LTCX`,
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			requiredOre := amountOfOreToProduceFuel(formulae, 1)
...
5 BHXH, 4 VRPVC => 5 LTCX`,
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			formulae := day14.ProcessInput(tt.input)
			requiredOre := maxFuelGenerated(formulae, trillion)
			if requiredOre != tt.maxFuel {
				t.Errorf("Expected %d, got %d", tt.maxFuel, requiredOre)
			}
		})
	}
}
